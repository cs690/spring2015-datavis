...
function drawGPLOM(data)
{
	...
	var drawYAxisCol3 = new numericalAxis().canvasID("#col3label").maxValue(maxValue).isHorizontal(true).height(30).margin({ top: 0, right: 10, bottom: 0, left: 10 });
	drawYAxisCol3();
	var drawYAxisCol4 = new numericalAxis().canvasID("#col4label").maxValue(maxValue).isHorizontal(true).height(30).margin({ top: 0, right: 10, bottom: 0, left: 10 });
	drawYAxisCol4();
	var drawYAxisRow3 = new numericalAxis().canvasID("#row3label").maxValue(maxValue).isHorizontal(false).width(40).margin({ top: 10, right: 1, bottom: 10, left: 20 });
	drawYAxisRow3();
	var drawYAxisRow4 = new numericalAxis().canvasID("#row4label").maxValue(maxValue).isHorizontal(false).width(40).margin({ top: 10, right: 1, bottom: 10, left: 20 });
	drawYAxisRow4();


	var drawXAxisCol1 = new numericalAxis().canvasID("#col1label").isCategorical(true)
	...
	drawXAxisCol1();
	var drawXAxisCol2 = new numericalAxis().canvasID("#col2label").isCategorical(true)
		...
		.colorMap(map);
	drawXAxisCol2();
	var drawXAxisRow1 = new numericalAxis().canvasID("#row1label").isCategorical(true)
    	.isHorizontal(false).isColor(true).categoryName("category").width(40).margin({ top: 10, right: 1, bottom: 10, left: 20 })
		.categoryValues(["1", "2", "3", "4", "5", "6", "7", "8"])
		.colorMap(map);
	drawXAxisRow1();
	var drawXAxisRow2 = new numericalAxis().canvasID("#row2label").isCategorical(true)
	...
	drawXAxisCol2.updateAxis("category", ["2", "4", "1", "5", "8", "7", "6", "3"]);
}
