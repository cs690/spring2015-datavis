...
function drawGPLOM(data)
{
	...
	var drawYAxis = new numericalAxis().canvasID("#cell12").maxValue(maxValue).isHorizontal(true);
	drawYAxis();

	var drawXAxis = new numericalAxis().canvasID("#cell13").isCategorical(true)
		.isHorizontal(true).isColor(true).categoryName("category")
		.categoryValues(["1","2","3","4","5","6","7","8"])
		.colorMap(map);
	drawXAxis();
	drawXAxis.updateAxis("category", ["2","4","1","5","8","7","6", "3"]);
}
