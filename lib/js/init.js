...
function drawGPLOM(data)
{
	...
	var barchart1 = barchart().width(size).height(size)
		...
		barchart3 = barchart().width(size).height(size)
			.numerical("comments").categorical("rate").categoryValues(starValues).categoryLabels(starLabels)
		...
		barchart6 = barchart().width(size).height(size)
			.numerical("ratings").categorical("rate").categoryValues(starValues).categoryLabels(starLabels)
		...
		barchart7 = barchart().width(size).height(size)
			...
			.tooltipYLabel("Average Video Length (sec)").tooltipXLabel("Views").svgID("barsvg7").canvasID("cell21")
			.offAxis(true).maxValue(maxValue).margin({ top: 40, right: 10, bottom: 10, left: 10 }),
		barchart8 = barchart().width(size).height(size)
			.numerical("length").categorical("category").categoryValues(categValues)
			.tooltipYLabel("Average Video Length (sec)").tooltipXLabel("Category").svgID("barsvg8").canvasID("cell22")
			.categoryLabels(categoryLabels).offAxis(true)
			.maxValue(maxValue).margin({ top: 40, right: 10, bottom: 10, left: 10 }),
		barchart9 = barchart().width(size).height(size)
			.numerical("length").categorical("rate").categoryValues(starValues).categoryLabels(starLabels)
			.tooltipYLabel("Average Video Length (sec)").tooltipXLabel("Stars").svgID("barsvg9").canvasID("cell23")
			.maxValue(maxValue).margin({ top: 40, right: 10, bottom: 10, left: 10 }).offAxis(true);
	...
	heatmap1.bindData(data, "views", viewValues, "category", categValues, "ratings").draw();
	heatmap2.bindData(data, "views", viewValues, "rate", starValues, "ratings").draw();
	heatmap3.bindData(data, "category", categValues, "rate", starValues, "ratings").draw();
	...
	var drawCategLegends = new CategoryLegends().canvasID("#cell2")
		...
		drawHeatmapLegends = new HeatmapLegend().canvasID("#cell8")
			.x(30).width(size).height(size).rectHeight(166).domain([0, 1847]).colors(DataHandler.prototype.getColorValues()).values(DataHandler.prototype.getColorDomain());
