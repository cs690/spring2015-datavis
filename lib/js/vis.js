    ...
    function chart(selection)
    {
        ...
        selection.each(function (data)
        {
            ...
            svg.selectAll("rect").data(processedData).enter().append("rect").
                ...
                {
                    commonOnEvent([categorical + d.index], false, true, false);
                })
                .on("mouseout", function (d)
                {
                    tip.hide();
                    commonLeaveEvent([categorical + d.index], false, true, false);
                })
                .transition()
                .duration(800)
...
function Heatmap(top, right, bottom, left, screenWidth, screenHeight)
{
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.draw = function ()
        {
            ...
            var vselection = svg.append("rect").attr("class", "selection selection-vertical").style("opacity", 0);
            var hselection = svg.append("rect").attr("class", "selection selection-vertical").style("opacity", 0);
            ...
            {
                commonOnEvent([that._axisX.data.name + d['attrX'], that._axisY.data.name + d['attrY']], false, false, true);
                ...
                tooltip
                .style("opacity", 1)
                .html("Average " + that._id.name[0].toUpperCase() + that._id.name.substring(1) + ": " + d['value'])
                .style("left", x + "px")
                .style("top", y + "px");
            })
            .on("mouseout", function (d)
...
function commonOnEvent(classes, isScatter, isBar, isHeatmap)
{
    ...
    {
        d3.selectAll("." + d).style("opacity", "1").style("stroke-width", "1px");
    });
    if (!isScatter) {
        d3.selectAll(".scatter").style("opacity", "0.05");//.style("visibility", "hidden");
        classes.forEach(function (d, i)
        {
            d3.selectAll(".s_" + d).style("fill", "red").style("opacity", "1");//.style("visibility", "visible");
        });
    }
}
...
function commonLeaveEvent(classes, isScatter, isBar, isHeatmap)
{
    ...
    if (!isScatter) {
        d3.selectAll(".scatter").style("fill", "gray").style("opacity", "1");//.style("visibility", "visible");
    }
}
