...
function ScatterPlot()
{
	...
	if (typeof this._initialized == "undefined")
	{
		...
		ScatterPlot.prototype.bindData = function(data, xAttr, yAttr, idAttr)
		{
			...
			this._id.value = function(d) { return d[idAttr];};
			
			this._axisX.name = xAttr;
			this._axisX.value = function(d) { return d[xAttr];};
			this._axisX.scale = d3.scale.linear().range([20, this._width+20]);
			...
			this._axisY.scale = d3.scale.linear().range([this._height-20, -20]),
		...
		ScatterPlot.prototype._drawTooltip = function(condition)
		{
			if(condition == "mouseover")
			{
				
			}
			else if(condition == "mouseout")
			{
...
function commonOnEvent (i) {
	...
	d3.select(".scatter" + i).style({opacity: 0.5});
	d3.select(".bar" + i).style({opacity: 0.5});
}

function commonLeaveEvent (i) {
	d3.select(".scatter" + i).style({opacity: 1.0});
	d3.select(".bar" + i).style({opacity: 1.0});	
}