    ...
    if (typeof this._initialized == "undefined")
    {
        ...
        this.draw = function ()
        {
			...
			.enter().append("rect")
			  ...
			  .attr("x", function (d) { return that._axisX.scale(that._categoryMap['attrX'][d['attrX']]) - that._axisX.scale.range()[1]; })
			  .attr("y", function (d) { return that._axisY.scale(that._categoryMap['attrY'][d['attrY']]) - that._axisY.scale.range()[that._axisY.categories.length - 2]; })
              .attr("width", this._axisX.scale.range()[1]*2)
              .attr("height", this._axisY.scale.range()[this._axisY.categories.length - 2]*2)
...
function commonOnEvent(classes, isScattor)
{
    ...
    if (!isScattor) {
        classes.forEach(function (d, i)
        {
            d3.selectAll(".s_" + d).style("opacity", "0.5");
        });
    }
}
...
function commonLeaveEvent(classes, isScattor)
{
    ...
    if (!isScattor) {
        classes.forEach(function (d, i)
        {
            d3.selectAll(".s_" + d).style("opacity", "1");
        });
    }
}
