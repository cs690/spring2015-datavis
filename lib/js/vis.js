function barchart() {
    var margin = {top:60, left:20, right:20, bottom:30},
...
function Heatmap(top, right, bottom, left, screenWidth, screenHeight)
{
    ...
    if (typeof this._initialized == "undefined")
    {
        ...
        {
            //catX = ["< 10000", "< 50000", "< 100000", ">= 100000"];
            //catY = ["0-1", "1-2", "2-3", "3-4", "4-5"];
            ...
            this._axisX.name = attrX;
            this._axisX.value = function (d) { return d[attrX]; };
            this._axisX.categories_origin = catX;
            console.log(this._axisX.categories_origin);
            //this._axisX.categories_map = function (v) { return that._axisX.categories_origin.indexOf(v); };
            this._axisX.categories_extend = dg.extendCategories(catX);
            console.log(this._axisX.categories_extend);
            this._axisX.scale = d3.scale.ordinal().domain(this._axisX.categories_extend).rangePoints([0, this._width]);
            this._axisX.map = function (d) { return that._axisX.scale(that._axisX.value(d)); };
            this._axisX.instance = d3.svg.axis().scale(this._axisX.scale).orient("bottom");
            this._axisX.instance.tickValues(this._axisX.scale.domain().filter(function (d, i) { return (i % 2); }));

            this._axisY.name = attrY;
            this._axisY.value = function (d) { return d[attrY]; };
            this._axisY.categories_origin = catY;
            //this._axisY.categories_map = function (v) { return that._axisY.categories_origin.indexOf(v); };
            this._axisY.categories_extend = dg.extendCategories(catY);
            this._axisY.scale = d3.scale.ordinal().domain(this._axisY.categories_extend).rangePoints([this._height, 0]);
            this._axisY.map = function (d) { return that._axisY.scale(that._axisY.value(d)); };
            this._axisY.instance = d3.svg.axis().scale(this._axisY.scale).orient("left");
            this._axisY.instance.tickValues(this._axisY.scale.domain().filter(function (d, i) { return (i % 2); }));
        ...
        this.draw = function ()
        {
			...
			.enter().append("rect")
			  ...
			  .attr("x", function (d) { return that._axisX.scale(that._axisX.categories_origin[+d['attrX'] - 1]) - that._axisX.scale.range()[1] + 2; })
			  .attr("y", function (d) { return that._axisY.scale(that._axisY.categories_origin[+d['attrY'] - 1]) - that._axisY.scale.range()[that._axisY.categories_extend.length - 2] + 2; })
              .attr("width", this._axisX.scale.range()[1] * 2 - 4)
              .attr("height", this._axisY.scale.range()[this._axisY.categories_extend.length - 2] * 2 - 4)
            ...
            .on("mousemove", function (d, i)
            {
                commonOnEvent([that._axisX.name + d['attrX'], that._axisY.name + d['attrY']], false);
                tooltip
                .style("opacity", 1)
            ...
            .on("mouseout", function (d, i)
            {
                commonLeaveEvent([that._axisX.name + d['attrX'], that._axisY.name + d['attrY']], false);
                tooltip
                .style("opacity", 0);
            });
...
function commonOnEvent(classes, isScattor)
{
    ...
    {
        d3.selectAll("." + d).style("opacity", "1").style("stroke-width", "2px").style("stroke", "black");
    });
    if (!isScattor) {
