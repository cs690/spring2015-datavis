...
function ScatterPlot(top, right, bottom, left, screenWidth, screenHeight)
{
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.updateAxis = function (attrName, maxValue)
        {
            ...
            this._canvas.select("." + axisName + ".axis").call(axis.instance);
            this._canvas.selectAll(".scatter").attr("c" + axisName, axis.data.mapFunc);
        }
...
function DataHandler(data)
{
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.findMaxValue = function (categoryName, numericals) {
            var data = this._data,
                maxList = [];
            ...
            numericals.forEach(function(numerical){
                ...
                values = values.map(function(d, i){ return d / counts[i]; });
