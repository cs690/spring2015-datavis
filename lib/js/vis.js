...
function ScatterPlot(top, right, bottom, left, screenWidth, screenHeight)
{
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.draw = function ()
        {
			...
			{
			    commonOnEvent(that._categories.map(function (v) { return v + d[v]; }), true, false, false);
			    ...
			    tooltip
                .style("opacity", 1)
                .html("Video ID: " + d[that._id.name] + "<br/>" + that._axisX.data.name[0].toUpperCase() + that._axisX.data.name.substring(1) + ": " + that._axisX.data.valueFunc(d) + "<br/>" + that._axisY.data.name[0].toUpperCase() + that._axisY.data.name.substring(1) + ": " + that._axisY.data.valueFunc(d))
                .style("left", x + "px")
                .style("top", y + "px");
			})
...
function Heatmap(top, right, bottom, left, screenWidth, screenHeight)
{
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.bindData = function (data, attrX, catX, attrY, catY, attrV)
        {
            ...
            this._id.color = d3.scale.linear().domain([this._heatData.maxValue, this._heatData.minValue]).range(['#006d2c', '#edf8fb']);
        ...
        this.draw = function ()
        {
            ...
            var vselection = svg.append("rect").attr("class", "selection x_" + this._axisX.data.name).style("opacity", 0);
            var hselection = svg.append("rect").attr("class", "selection y_" + this._axisY.data.name).style("opacity", 0);
            ...
            .on("mouseenter", function (d)
            {
                commonOnEvent([that._axisX.data.name + '_' + d['attrX'], that._axisY.data.name + '_' + d['attrY']], false, false, true);

                drawVSelection(this, vselection);
                drawHSelection(this, hselection);

                //var t =  d3.select(this);
                //var vx = t.attr("x");
                //var vw = t.attr("width");
                //vselection.attr("x", +vx - 1).attr("y", 0).attr("width", +vw + 2).attr("height", 182).style("opacity", 1);
                //var vy = t.attr("y");
                //var vh = t.attr("height");
                //hselection.attr("x", 0).attr("y", +vy-1).attr("width", 182).attr("height", +vh+2).style("opacity", 1);
                ...
                tooltip
                .style("opacity", 1)
                .html("Video Group: " + that._axisX.data.categoryValueFunc(d['attrX']) + " and " + that._axisY.data.categoryValueFunc(d['attrY']) + "<br>Video Count: " + d['count'] + "<br>Average " + that._id.name[0].toUpperCase() + that._id.name.substring(1) + ": " + d['value'])
                .style("left", x + "px")
                .style("top", y + "px");
            })
            .on("mouseout", function (d)
...
function drawVSelection(dom, selection)
{
    var t = d3.select(dom);
    var vx = t.attr("x");
    var vw = t.attr("width");
    selection.attr("x", +vx - 1).attr("y", 0).attr("width", +vw + 2).attr("height", 182).style("opacity", 1);
}

function drawHSelection(dom, selection)
{
    var t = d3.select(dom);
    var vy = t.attr("y");
    var vh = t.attr("height");
    selection.attr("x", 0).attr("y", +vy - 1).attr("width", 182).attr("height", +vh + 2).style("opacity", 1);
}
    ...
    function draw()
    {
        ...
        if (isColor)
            svg.selectAll("rect")
                .data(colorMap).enter()
                .append("rect")
                .attr("class", function (d, i) { return "category" + (i + 1); })
...
function TextBox () {
    ...
    function write () {
        ...
        var svg = d3.select((canvasID.charAt(0) === "#")? "" : "#" + canvasID)
            .append("svg").attr("height", height).attr("width", width);

        svg.append("text").attr("x", x).attr("y", y).html(html);
    }

    write.x = function (value) { x = value; return write; }
    write.y = function (value) { y = value; return write; }
    write.html = function (value) { html = value; return write; }
    write.canvasID = function (value) { canvasID = value; return write; }
    write.height = function (value) { height = value; return write; }
    write.width = function (value) { width = value; return write; }
...
function notify(categoryType)
{
    ...
    switch (categoryType.toLowerCase()) {
        case "category":
            ...
            break;
        case "views":
            message = "'Views': Reproduction of videos";
            break;
        case "stars":
            message = "'Stars': User ratings of videos";
            break;
        case "ratings":
            ...
            break;
        case "comments":
            message = "'Comments': Number of people's comments, not letters of comments";
            timeout = 5500;
            break;
        case "length":
            message = "'Length': Video length";
            break;
        case "gplom":
