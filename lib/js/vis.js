    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.draw = function ()
        {
			...
			.on("mousemove", function (d, i)
			{
			    ...
			    tooltip
                .style("opacity", 1)
                .html(that._axisX.data.name[0].toUpperCase() + that._axisX.data.name.substring(1) + " : " + that._axisX.data.valueFunc(d) + "<br/>" + that._axisY.data.name[0].toUpperCase() + that._axisY.data.name.substring(1) + " : " + that._axisY.data.valueFunc(d))
                .style("left", x + "px")
                .style("top", y + "px");
			})
...
function commonOnEvent(classes, isScatter)
{
...
function commonLeaveEvent(classes, isScatter)
{
