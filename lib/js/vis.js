    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.draw = function ()
        {
			...
			.on("mouseover", function (d, i)
			{
			    ...
			    tooltip
                .style("opacity", 1)
                .html("Video ID: " + d[that._id.name] + "<br/>" + initialToUppercase(that._axisX.data.name) + ": " + that._axisX.data.valueFunc(d) + "<br/>" + initialToUppercase(that._axisY.data.name) + ": " + that._axisY.data.valueFunc(d))
                .style("left", x + "px")
                .style("top", y + "px");
			})
...
function Heatmap(top, right, bottom, left, screenWidth, screenHeight)
{
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.bindData = function (data, attrX, catX, attrY, catY, attrV)
        {
            ...
            this._id.color = d3.scale.linear().domain([1, 1846]).range(['#edf8fb', '#006d2c']);
        ...
        this.draw = function ()
        {
            ...
            .on("mouseover", function (d)
            {
                ...
                tooltip
                .style("opacity", 1)
                .html(initialToUppercase(that._axisX.data.name) + ": " + that._axisX.data.categoryValueFunc(d['attrX']) + "<br>"+initialToUppercase(that._axisY.data.name) + ": " + that._axisY.data.categoryValueFunc(d['attrY']) + "<br>Video Count: " + d['count'] + "<br>Average " + initialToUppercase(that._id.name) + ": " + d['value'])
                .style("left", x + "px")
                .style("top", y + "px");
            })
            .on("mouseout", function (d)
...
function commonOnEvent(classes, isScatter, isBar, isHeatmap)
{
    ...
    if (isHeatmap) {
        ...
        classes.forEach(function (d)
        {
            ...
            d3.selectAll('.' + d).style("stroke-width", "1px").style("stroke", "black");
        });
    }
    else if (isBar) {
        d3.selectAll(".scatter").style("opacity", "0.05");
        ...
        d3.selectAll('.' + classes[0]).style("stroke-width", "1px").style("stroke", "black");
    }
    else if (isScatter) {
        d3.selectAll(".bar").style("opacity", "0.1");
        //console.log(classes);
...
function commonLeaveEvent(classes, isScatter, isBar, isHeatmap)
{
    if (isHeatmap) {
        ...
        d3.selectAll(".legend").style("stroke", "none");
    }
    else if (isBar) {
        d3.selectAll(".bar").style("opacity", "1").style("fill", "gray");
        d3.selectAll(".scatter").style("opacity", "1").style("fill", "gray");
    ...
    function draw()
    {
        ...
        if (isColor)
            svg.selectAll("rect")
                .data(colorMap).enter()
                .append("rect")
                .attr("class", function (d, i) { return "legend category_" + (i + 1); })
...
function HeatmapLegend () {
    ...
    draw.height = function (value) { height = value; return draw; }
    draw.width = function (value) { width = value; return draw; }
    draw.maxColor = function (value) { maxColor = value; return draw; }
    draw.minColor = function (value) { minColor = value; return draw; }
