...
function ScatterPlot()
{
	...
	if (typeof this._initialized == "undefined")
	{
		...
		ScatterPlot.prototype.drawOn = function(data, xAttr, yAttr, idAttr)
		{
			...
			var yValue = function(d) { return d[yAttr];},
				yScale = d3.scale.linear().range([this._height, 0]),
			...
			svg.selectAll(".scattor")
			  .data(data)
			.enter().append("circle")
			  .attr("class", function(d) { return "scattor " + idValue(d);})
			  .attr("r", 2.5)
			  .attr("cx", xMap)
			  .attr("cy", yMap)
			  .style("fill", "#3498db")
