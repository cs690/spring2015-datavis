...
function Heatmap(top, right, bottom, left, screenWidth, screenHeight)
{
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.bindData = function (data, attrX, catX, attrY, catY, attrV)
        {
            ...
            this._axisX.name = attrX;
            this._axisX.value = function (d) { return d[attrX]; };
            this._axisX.categories = catX;
            this._axisX.scale = d3.scale.ordinal().domain(this._axisX.categories).rangeRoundBands([0, this._width], 0.05, 0.05);
            this._axisX.map = function (d) { return that._axisX.scale(that._axisX.value(d)); };
            this._axisX.instance = d3.svg.axis().scale(this._axisX.scale).orient("bottom");

            this._axisY.name = attrY;
            this._axisY.value = function (d) { return d[attrY]; };
            this._axisY.categories = catY;
            this._axisY.scale = d3.scale.ordinal().domain(this._axisY.categories).rangeRoundBands([this._height, 0], 0.05, 0.05);
            this._axisY.map = function (d) { return that._axisY.scale(that._axisY.value(d)); };
            this._axisY.instance = d3.svg.axis().scale(this._axisY.scale).orient("left");
        ...
        this.draw = function ()
        {
            ...
            .attr("x", function (d) { return that._axisX.scale(that._axisX.categories[+d['attrX'] - 1]); })
            .attr("y", function (d) { return that._axisY.scale(that._axisY.categories[+d['attrY'] - 1]); })
            .attr("width", this._axisX.scale.rangeBand())
            .attr("height", this._axisY.scale.rangeBand())
        ...
        this.updateAxis = function (attrName, newOrder)
        {
            ...
            var axisName = "";
            switch (attrName) {
                case this._axisX.name:
                    axis = this._axisX;
                    axisName = "x";
                    break;
                case this._axisY.name:
                    axis = this._axisY;
                    axisName = "y";
                    break;
                default:
            ...
            d3.selectAll(".rect").transition().duration(750).attr(axisName, function (d) { return axis.scale(axis.categories[+d['attr' + axisName.toUpperCase()] - 1]); });
        }
