function barchart()
{
    ...
    chart.height = function (value) { height = value; return chart; };
    chart.x = function (value) { xValue = value; return chart; };
    chart.y = function (value) { yValue = value; return chart; }
    chart.yLabel = function (value) { yLabel = value; return chart; }
    chart.numerical = function (value) { numerical = value; return chart; }
    chart.categorical = function (value) { categorical = value; return chart;}
    chart.tooltipXLabel = function (value) { tooltipXLabel = value; return chart; }
    chart.offAxis = function (value) { offAxis = value; return chart; }
    chart.tooltipYLabel = function (value) { tooltipYLabel = value; return chart; }
    chart.svgID = function (value) { svgID = value; return chart; }
    chart.canvasID = function (value) { canvasID = value; return chart; }
    chart.labelFontSize = function (value) { labelFontSize = value; return chart; }
    chart.categoryValues = function (values) { categoryValues = values; return chart; }
    chart.categoryLabels = function (values) { categoryLabels = values; return chart; }
    chart.maxValue = function (value) { maxVal = value; return chart; }
    chart.margin = function (value) { margin = value; return chart; }
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.draw = function ()
        {
            ...
            .on("mouseover", function (d, i)
            {
                ...
                tooltip
                .style("display", "block")
                .html("Video ID: <a href=\"https://www.youtube.com/watch?v=" + d[that._id.name] + "\">" + d[that._id.name] + "</a><br/>" + DataHandler.prototype.initialToUppercase(that._axisX.data.name) + ": " + that._axisX.data.valueFunc(d) + "<br/>" + DataHandler.prototype.initialToUppercase(that._axisY.data.name) + ": " + that._axisY.data.valueFunc(d))
                .style("left", x + "px")
                .style("top", y + "px");
            })
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.draw = function ()
        {
            ...
            .on("mouseover", function (d)
            {
                ...
                var labelX = DataHandler.prototype.initialToUppercase(that._axisX.data.name);
                var labelXValue = that._axisX.data.categoryValueFunc(d['attrX']);
                if (labelX == 'Rate') {
                    labelX = 'Stars';
                    labelXValue = DataHandler.prototype.mapStars(labelXValue);
                }
                else if (labelX == 'Category')
                {
                    labelXValue = DataHandler.prototype.mapCategories(labelXValue);
                }
                var labelY = DataHandler.prototype.initialToUppercase(that._axisY.data.name);
                var labelYValue = that._axisY.data.categoryValueFunc(d['attrY']);
                if (labelY == 'Rate') {
                    labelY = 'Stars';
                    labelYValue = DataHandler.prototype.mapStars(labelYValue);
                }
                else if (labelY == 'Category') {
                    labelYValue = DataHandler.prototype.mapCategories(labelYValue);
                }
                tooltip
                .style("display", "block")
                .html(labelX + ": " + labelXValue + "<br>" + labelY + ": " + labelYValue + "<br>Video Count: " + d['count'] + "<br>Average " + DataHandler.prototype.initialToUppercase(that._id.name) + ": " + d['value'])
                .style("left", x + "px")
                .style("top", y + "px");
            })
            .on("mouseout", function (d)
...
function commonOnEvent(classes, isScatter, isBar, isHeatmap)
{
    ...
    var b_cstr = DataHandler.prototype.buildClassString(classes, 'b_');
    var s_cstr = DataHandler.prototype.buildClassString(classes, 's_');
    var h_cstr = DataHandler.prototype.buildClassString(classes, 'h_');
    var l_cstr = DataHandler.prototype.buildClassString(classes, 'l_');
    if (isHeatmap) {
...
function DataHandler(data)
{
    ...
    if (typeof this._initialized == "undefined") {
        ...
        DataHandler.prototype.mapCategories = function (value)
        {
            switch(value)
            {
                case "\u2460": return "Comedy";
                case "\u2461": return "Entertainment";
                case "\u2462": return "Film & Animation";
                case "\u2463": return "Music";
                case "\u2464": return "News & Politics";
                case "\u2465": return "People & Blogs";
                case "\u2466": return "Sports";
                case "\u2467": return "Others";
            }
        }

        DataHandler.prototype.buildClassString = function(classes, prefix)
        {
            var cstr = '';
        ...
        DataHandler.prototype.initialToUppercase = function (name)
        {
            return name[0].toUpperCase() + name.substring(1);
        }

        DataHandler.prototype.getOriginalClasses = function (classes)
        {
            var result = [];
            for (var i = 0, l = classes.length; i < l; i++) {
...
function notify(categoryType)
{
    ...
    switch (categoryType.toLowerCase()) {
        ...
        case "stars":
            ...
            break;
        case "ratings":
            message = "'Ratings': Number of people's ratings (i.e. How many times people have rated the video)";
            timeout = 5500;
            break;
        case "comments":
