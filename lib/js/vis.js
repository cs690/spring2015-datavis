...
function Heatmap(top, right, bottom, left, screenWidth, screenHeight)
{
    ...
    if (typeof this._initialized == "undefined") {
        ...
        this.draw = function ()
        {
            ...
            .attr("x", function (d) { return that._axisX.data.scale(that._axisX.data.categories[+d['attrX'] - 1]); })
            .attr("y", function (d) { return that._axisY.data.scale(that._axisY.data.categories[+d['attrY'] - 1]); })
